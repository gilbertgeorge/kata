...
// this is the wild west, with dreadful weather and not much water, it's important to save yourself some energy, otherwise you might die of thirst!
//
...
// or
// [North, South, South, East, West, North, West]
// You can immediately see that going "NORTH" and immediately "SOUTH" is not reasonable, better stay to
// the same place! So the task is to give to the man a simplified version of the plan. A better plan in this case is simply:
//
// ["WEST"]
// or
// { "WEST" }
...
export function dirReduc(arr: string[]): string[] {
    ...
    for (const dir of arr) {
        if (result.length > 0 && result[result.length - 1] === opposites[dir as keyof typeof opposites]) {
            result.pop();
        } else {
            result.push(dir);
        }
